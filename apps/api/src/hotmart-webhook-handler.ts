...
import type { HotmartWebhookBody } from '@hub/shared';
import { prisma } from './db.js';

export type HotmartHandlerDeps = {
...
> {
  ...
  const secret = (deps.getSecret ?? (() => process.env.HOTMART_WEBHOOK_SECRET))();
  ...
  // Format: SHA256(tenantId | purchase.id | "purchase" | purchase.price | currency)
  const purchaseId = body.purchase?.id || body.id || 'unknown';
  const amount = body.purchase?.price || body.purchase?.full_price || 0;
  const currency = body.purchase?.currency || 'BRL';
  const eventId = sha256hex(`${tenantId}|${purchaseId}|purchase|${amount}|${currency}`);
  ...
  const insertDedupe = deps.insertDedupe ?? (async (data) => {
    try {
      ...
      return false; // UNIQUE constraint violation = duplicata
    }
  });
