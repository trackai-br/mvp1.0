...
import { normalizeHotmartConversion, createIdentityHashes } from './utils/normalize-conversion.js';
...
> {
  ...
  const normalized = normalizeHotmartConversion(body);
  ...
  if (isNew) {
    try {
      ...
      if (matchResult.success && matchResult.conversionId) {
        ...
        console.log(`[hotmart-webhook] ✓ Conversion created and matched (score: ${matchScore})`);
      }
    } catch (error) {
      console.error('[hotmart-webhook] Error in matching engine:', error);
    }
  }
